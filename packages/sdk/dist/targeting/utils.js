...
exports.getTargetingGuidance = getTargetingGuidance;
/**
 ...
 */
function getTargetingGuidance(warnings) {
    if (warnings.length === 0)
        return "";
    const guidance = [];
    for (const warning of warnings) {
        ...
        if (reason === "not-found") {
            guidance.push("Ensure the element exists in the DOM and has the correct data-elementid attribute or verify the selector");
        }
        else if (reason === "invalid-selector") {
            guidance.push("Check that the CSS selector syntax is valid and properly escaped");
        }
        else if (reason === "no-target-provided") {
            guidance.push("Provide either elementId or selector in the command payload");
        }
    }
